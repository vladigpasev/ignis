...
const BASE_URL = process.env.APP_BASE_URL || 'https://firelinks.org';
...
async function fetchFirms(bbox: [number, number, number, number]) {
  const [w, s, e, n] = bbox;
  const url = `${BASE_URL}/api/firms?bbox=${w},${s},${e},${n}&days=1&minConfidence=35&dedupRadiusM=600&clusterRadiusM=650`;
...
export async function GET(req: Request) {
  try {
    ...
    const onlyId = Number(url.searchParams.get('subscriptionId') || url.searchParams.get('id'));
    const limitPerSource = Math.max(1, Math.min(10, Number(url.searchParams.get('limitPerSource')) || 3));

    const subs = await (async () => {
      if (Number.isFinite(onlyId)) {
        const [s] = await db.select().from(notificationSubscriptions).where(eq(notificationSubscriptions.id, onlyId)).limit(1);
        return s ? [s] : [];
      }
    ...
    for (const sub of subs) {
      ...
      if ((sub.sourceReports ?? 1) === 1) {
        ...
        for (const f of rows) {
          ...
          const { subject, html, text } = buildReportEmail({ lat: f.lat, lng: f.lng, radiusM: f.radiusM, fireId: f.id, baseUrl: BASE_URL });
          const smsMsg = buildReportSms({ lat: f.lat, lng: f.lng, fireId: f.id, baseUrl: BASE_URL });
      ...
      if ((sub.sourceFirms ?? 1) === 1) {
        let sent = 0;
        try {
          const hotspots = await fetchFirms(bbox);
          for (const h of hotspots) {
            const d = haversineMeters({ lat: sub.lat, lng: sub.lng }, { lat: h.lat, lng: h.lng });
            if (d > radiusM) continue;
            const eventKey = `firms:${h.id}`;
            totalCandidates++;
            if (await alreadyDelivered(sub.id, eventKey)) continue;

            const { subject, html, text } = buildReportEmail({ lat: h.lat, lng: h.lng, radiusM: h.radiusM, baseUrl: BASE_URL });
            const smsMsg = buildReportSms({ lat: h.lat, lng: h.lng, baseUrl: BASE_URL });
          ...
          console.warn('[webhook firms] failed', (e as any)?.message);
        }
      }
    }

    return NextResponse.json({ ok: true, totalSubscriptions: subs.length, totalCandidates, totalDelivered });
  } catch (e: any) {
    console.error(e);
