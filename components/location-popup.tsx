      ...
      <div className="w-[300px] sm:w-[350px]">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {brand && brand !== name && <p className="text-sm font-medium text-muted-foreground">{brand}</p>}
            {address && (
        ...
        {categories.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1 max-w-full">
            {categories.slice(0, 3).map((category, index) => (
              <Badge key={index} variant="secondary" className="text-xs capitalize truncate max-w-[100px]">
                {category}
              </Badge>
            ))}
            {categories.length > 3 && (
        ...
        <div className="mt-3 pt-2 border-t text-xs text-muted-foreground">
          <div className="flex justify-between items-center">
            <span className="truncate max-w-[170px]">ID: {properties?.mapbox_id?.substring(0, 8)}...</span>
